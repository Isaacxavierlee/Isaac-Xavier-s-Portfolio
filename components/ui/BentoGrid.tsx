...
import { cn } from "@/utils/cn"; // Make sure cn is imported properly
  ...
  return (
    <div
      ...
      style={{
        ...
        maxWidth: "100%", // Ensures the container stays within the viewport
      }}
    >
      {/* Image Container */}
      <div className={`${id === 6 ? "flex justify-center" : "w-full"} h-full absolute`}>
      ...
      <div
        ...
        style={{ maxWidth: "100%" }} // Ensures the content stays within the viewport
      >
      ...
      {/* Tech Stack */}
      {id === 3 && (
        <div className="flex gap-1 lg:gap-5 w-full md:w-auto">
          <div className="flex flex-wrap gap-3 md:gap-3 lg:gap-8">
            {['React.js', 'Next.js', 'TypeScript'].map((item, index) => (
              <span key={index} className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-base opacity-100 lg:opacity-100 rounded-lg text-center bg-[#10132E] text-white">
                {item}
              </span>
            ))}
          </div>
